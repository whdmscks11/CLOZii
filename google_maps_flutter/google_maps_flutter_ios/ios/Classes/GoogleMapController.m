...
@property(nonatomic, strong) FlutterMethodChannel *methodChannel; // 추가 
@property(nonatomic, assign) long long mapId; // 추가 
...
- (instancetype)initWithFrame:(CGRect)frame
  ...
  if (self) {
    ...
    // ✅ MethodChannel 생성
    NSString *chName = [NSString stringWithFormat:@"plugins.flutter.io/google_maps_%lld", _mapId];
    _methodChannel = [FlutterMethodChannel methodChannelWithName:chName
                                                binaryMessenger:[registrar messenger]];
  }

  return [self initWithMapView:_mapView
                viewIdentifier:viewId
...
didTapPOIWithPlaceID:(NSString *)placeID
  ...
  // FGM 접두어 타입 사용 + 팩토리로 생성
  FGMPlatformLatLng *pos =
      [FGMPlatformLatLng makeWithLatitude:location.latitude longitude:location.longitude];

  FGMPlatformPoi *poi =
      [FGMPlatformPoi makeWithPlaceId:placeID name:name position:pos];

  // 생성된 헤더(messages.g.h)에서 정확한 셀렉터 확인!
  // 보통 아래 둘 중 하나:
  // [self.dartCallbackHandler onPoiTap:poi completion:^(FlutterError *_Nullable error) {}];
  // [self.dartCallbackHandler onPoiTapPoi:poi completion:^(FlutterError *_Nullable error) {}];

  // ✅ 헤더에 나온 정확한 셀렉터 사용!
  [self.dartCallbackHandler onPoiTapPoi:poi completion:^(FlutterError *_Nullable error) {
    if (error) {
      NSLog(@"onPoiTapPoi error: %@", error);
    }
  }];
}
...
//   [self.methodChannel invokeMethod:@"poi#tap" arguments:args];
// }
