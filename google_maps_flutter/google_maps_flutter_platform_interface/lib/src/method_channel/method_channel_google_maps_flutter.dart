...
class MethodChannelGoogleMapsFlutter extends GoogleMapsFlutterPlatform {
  final Map<int, StreamController<PoiTapEvent>> _poiTapControllers =
      <int, StreamController<PoiTapEvent>>{};
  ...
  void dispose({required int mapId}) {
    _poiTapControllers.remove(mapId)?.close();
    _tileOverlays.remove(mapId);     // ✅ 타일 오버레이 맵 비우기
  _channels.remove(mapId);         // ✅ 채널 맵에서도 제거
    // Noop!
  }
  ...
  Stream<PoiTapEvent> onPoiTap({required int mapId}) {
    return (_poiTapControllers[mapId] ??=
            StreamController<PoiTapEvent>.broadcast())
        .stream;
  }
  ...
  Future<dynamic> _handleMethodCall(MethodCall call, int mapId) async {
    switch (call.method) {
      ...
      case 'poi#tap':
        {
          final int mapId = (call.arguments['mapId'] as num).toInt();
          final Map<dynamic, dynamic> poiMap =
              call.arguments['poi'] as Map<dynamic, dynamic>;
          final poi = PointOfInterest.fromMap(poiMap);
          final controller = _poiTapControllers[mapId];
          if (controller != null && !controller.isClosed) {
            controller.add(PoiTapEvent(mapId, poi));
          }
        }
